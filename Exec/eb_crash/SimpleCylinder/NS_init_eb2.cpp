		...
		const int max_coarsening_level)
{
  ...
  else if (geom_type == "GoodPiston")
  {
    ...
    p = amrex::RealVect(D_DECL(42.0*0.1*scaleFact, -15.0*0.1*scaleFact, 0.0));
    lnpts.push_back(p);
    p = amrex::RealVect(D_DECL(0.*0.1*scaleFact, -15.0*0.1*scaleFact, 0.0));
    lnpts.push_back(p);
    Piston.addLineElement(lnpts);

    EB2::CylinderIF cylinder(40.0*0.1*scaleFact, 60.0*0.1*scaleFact, 2, {0.0, 0.0, 0.0*0.1*scaleFact}, true);
  ...
  else if (geom_type == "BadPiston")
  {
    ...
    p = amrex::RealVect(D_DECL(42.0*0.1*scaleFact, -15.2*0.1*scaleFact, 0.0));
    lnpts.push_back(p);
    p = amrex::RealVect(D_DECL(0.*0.1*scaleFact, -15.2*0.1*scaleFact, 0.0));
    lnpts.push_back(p);
    Piston.addLineElement(lnpts);
