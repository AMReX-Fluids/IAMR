...
#pragma omp parallel  if (Gpu::notInLaunchRegion())
	...
	{
	  init_userDefined(vbx, P_new.array(mfi), S_new.array(mfi, Xvel),
			   S_new.array(mfi, Density), nscal,
			   domain, dx, problo, probhi, IC);
	}
	else
...
void NavierStokes::init_userDefined (Box const& vbx,
			       Array4<Real> const& press,
			       Array4<Real> const& vel,
			       Array4<Real> const& scal,
			       const int nscal,
			       ...
			       InitialConditions IC)
{
...
#if (AMREX_SPACEDIM != 3)
    amrex::Abort("NavierStokes::init_UserDefined: This is a 3D problem, please recompile with DIM=3 in makefile");
#endif
  ...
  amrex::ParallelFor(vbx, [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
  {
    ...
    Real angular_vel = r_xy * 150.0 * std::tanh(300*(0.02-r_xy));
