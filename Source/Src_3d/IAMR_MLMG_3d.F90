...
contains
  ...
  subroutine iamr_mac_coef (xlo, xhi, ylo, yhi, zlo, zhi, bx, bxlo, bxhi, by, bylo, byhi, &
    ...
    do       k = xlo(3), xhi(3)
       do    j = xlo(2), xhi(2)
          do i = xlo(1), xhi(1)
             bx(i,j,k) = 2.d0 / (rho(i-1,j,k)+rho(i,j,k))
          end do
       end do
    end do
    ...
    do       k = ylo(3), yhi(3)
       do    j = ylo(2), yhi(2)
          do i = ylo(1), yhi(1)
             by(i,j,k) = 2.d0 / (rho(i,j-1,k)+rho(i,j,k))
          end do
       end do
    end do
    ...
    do       k = zlo(3), zhi(3)
       do    j = zlo(2), zhi(2)
          do i = zlo(1), zhi(1)
             bz(i,j,k) = 2.d0 / (rho(i,j,k-1)+rho(i,j,k))
          end do
       end do
    end do
  ...
  subroutine iamr_mac_rhs (lo, hi, rhs, rlo, rhi, ux, uxlo, uxhi, uy, uylo, uyhi, &
       uz, uzlo, uzhi, dxinv, scale) bind(c,name='iamr_mac_rhs')
    integer, dimension(3) :: lo, hi, rlo, rhi, uxlo, uxhi, uylo, uyhi, uzlo, uzhi
    real(amrex_real), intent(in) :: scale, dxinv(3)
    ...
    do       k = lo(3), hi(3)
       do    j = lo(2), hi(2)
          do i = lo(1), hi(1)
             ...
             rhs(i,j,k) = scale*(rhs(i,j,k) - divu)
          end do
       end do
    end do
