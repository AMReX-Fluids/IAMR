...
contains
  ...
  subroutine iamr_mac_coef (xlo, xhi, ylo, yhi, bx, bxlo, bxhi, by, bylo, byhi, rho, rlo, rhi) &
    ...
    do    j = xlo(2), xhi(2)
       do i = xlo(1), xhi(1)
          bx(i,j) = 2.d0 / (rho(i-1,j)+rho(i,j))
       end do
    end do

    do    j = ylo(2), yhi(2)
       do i = ylo(1), yhi(1)
          by(i,j) = 2.d0 / (rho(i,j-1)+rho(i,j))
       end do
    end do
  ...
  subroutine iamr_mac_rhs (lo, hi, rhs, rlo, rhi, ux, uxlo, uxhi, uy, uylo, uyhi, &
       ax, axlo, axhi, ay, aylo, ayhi, vol, vlo, vhi, scale) bind(c,name='iamr_mac_rhs')
