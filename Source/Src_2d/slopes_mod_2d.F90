...
contains
   ...
   subroutine compute_slopes ( lo, hi, nc, vel, vlo, vhi, &
      ...
      ! Arrays
      real(ar),       intent(in   ) ::                      &
           & vel(vlo(1):vhi(1),vlo(2):vhi(2),nc)
      ...
      do n = 1, nc
         do j = lo(2), hi(2)
            do i = lo(1), hi(1)
               ...
               ! X direction
               du_l = two*(vel(i  ,j,n) - vel(i-1,j,n))
               du_r = two*(vel(i+1,j,n) - vel(i  ,j,n))
               du_c = half * ( vel(i+1,j,n) - vel(i-1,j,n) )
               xslopes(i,j,n) = mc_limiter ( du_l, du_c, du_r )

               ! Y direction
               du_l = two*(vel(i,j  ,n) - vel(i,j-1,n))
      ...
      !          if ( ( bc_ilo_type(j,1) == MINF_ ) .or. &
      !              ( bc_ilo_type(j,1) == NSW_ )  .or. &
      !              ( bc_ilo_type(j,1) == FSW_ )  .or. &
      !              ( bc_ilo_type(j,1) == PSW_ )  ) then

      !             du_l = two*(vel(i  ,j,n) - vel(i-1,j,n))
      !             du_r = two*(vel(i+1,j,n) - vel(i  ,j,n))
      !             du_c = (vel(i+1,j,n)+3.d0*vel(i,j,n)-4.d0*vel(i-1,j,n))/3.d0
      !             xslopes(i,j,n) = mc_limiter ( du_l, du_c, du_r )
      ...
      !          if ( ( bc_ihi_type(j,1) == MINF_ ) .or. &
      !              ( bc_ihi_type(j,1) == NSW_ )  .or. &
      !              ( bc_ihi_type(j,1) == FSW_ )  .or. &
      !              ( bc_ihi_type(j,1) == PSW_ )  ) then
      ...
      !          if ( ( bc_jlo_type(i,1) == MINF_ ) .or. &
      !              ( bc_jlo_type(i,1) == NSW_ )  .or. &
      !              ( bc_jlo_type(i,1) == FSW_ )  .or. &
      !              ( bc_jlo_type(i,1) == PSW_ )  ) then

      !             du_l = two*(vel(i,j  ,n) - vel(i,j-1,n))
      !             du_r = two*(vel(i,j+1,n) - vel(i,j  ,n))
      !             du_c = (vel(i,j+1,n)+3.d0*vel(i,j,n)-4.d0*vel(i,j-1,n))/3.d0
      ...
      !          if ( ( bc_jhi_type(i,1) == MINF_ ) .or. &
      !              ( bc_jhi_type(i,1) == NSW_ )  .or. &
      !              ( bc_jhi_type(i,1) == FSW_ )  .or. &
      !              ( bc_jhi_type(i,1) == PSW_ )  ) then
         ...
         do j = lo(2), hi(2)
            do i = lo(1), hi(1)
               ...
               if ( is_covered_cell(flags(i,j)) ) then
                  xslopes(i,j,n) = huge(one)
                  yslopes(i,j,n) = huge(one)
               else
                  ! X direction
                  du_l = two*(vel(i  ,j,n) - vel(i-1,j,n))
                  du_l = merge( zero, du_l, is_covered_cell(flags(i-1,j)) )

                  du_r = two*(vel(i+1,j,n) - vel(i  ,j,n))
                  du_r = merge( zero, du_r, is_covered_cell(flags(i+1,j)) )
                  ...
                  ! Y direction
                  du_l = two*(vel(i,j  ,n) - vel(i,j-1,n))
                  du_l = merge( zero, du_l, is_covered_cell(flags(i,j-1)) )

                  du_r = two*(vel(i,j+1,n) - vel(i,j  ,n))
                  du_r = merge( zero, du_r, is_covered_cell(flags(i,j+1)) )
